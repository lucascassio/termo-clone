...
async function verifyVictory(wordGuess) {
  if (wordGuess === word) {
...
async function verifyExistance(wordGuess) {
  const terms = await fetchTerms();
  return terms.some(term => term.palavra === wordGuess);
}
...
async function verifyWord() {
  try {
    let wordGuess = unifyWord(userGuess);
    let exists = await verifyExistance(wordGuess);
